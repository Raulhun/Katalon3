...
if (CountResults >= 1) {
    ...
    if (WebUI.waitForElementVisible(findTestObject('Alert_Messages/div_torles_nem_lehetseges'), 8)) {
        ...
        KeywordUtil.markWarning('A paciensnek fuggosegei vannak, ezert nem törölhető')
    }
    
    WebUI.delay(4)
} else {
    KeywordUtil.markWarning('A kereses nem hozott eredmenyt')
}

KeywordUtil.markWarning('A keresés törlés csekk működik!')
